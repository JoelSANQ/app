...
int total() {
  ...
  List<int> a = []; // Lista vacía

  for (var i = 0; i < history2.length; i++) {
    int amount = int.tryParse(history2[i].amount) ?? 0;
    a.add(history2[i].IN == 'Ingreso' ? amount : - amount);
  }

  totals = a.fold(0, (value, element) => value + element); // Manejo seguro
  return totals;
}
...
int income() {
  ...
  List a = [0, 0];
  for (var i = 0; i < history2.length; i++) {
    a.add(history2[i].IN == 'Ingreso' ? int.parse(history2[i].amount) : 0);
  }
...
int expenses() {
  ...
  List a = [0, 0];
  for (var i = 0; i < history2.length; i++) {
    a.add(history2[i].IN == 'Ingreso' ? 0 : int.parse(history2[i].amount) * -1);
  }
  totals = a.reduce((value, element) => value + element);
  return totals;
}
